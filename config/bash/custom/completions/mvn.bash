...
__find_mvn_projects() {
	find . -name 'pom.xml' -not -path '*/target/*' -prune | while read -r LINE; do
...
__pom_hierarchy() {
	...
	while [ -n "$pom" ] && grep -q "<parent>" "$pom"; do
		...
		parent_pom_relative=$(grep -e "<relativePath>.*</relativePath>" "$pom" | sed 's/.*<relativePath>//' | sed 's/<\/relativePath>.*//g')
