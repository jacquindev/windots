_vboxmanage_list() {
	...
	WORDS=$(gawk <<<"$res" '/[[:graph:]]/{ a[i++] = $0 } END {
            if (isarray(a)) {
                len = length(i)
                for (i in a)
                    printf "%0*d) %s\n", len, i+1, a[i]
...
_vboxmanage_option() {
	...
	if [[ $CMD2 == @(snapshot|encryptvm|controlvm|debugvm|modifynvram|bandwidthctl|guestcontrol) ]]; then
		if [[ -n $CMD3 ]]; then
			HELP=$(sed <<<"$HELP" -En "/$CMD $CMD2 [^ ]+ $CMD3/p")
		else
			HELP=""
		fi
		...
		if [[ -z $CMD3 ]]; then
			HELP=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"' (.*) \w{3,}( .*|$)/\1/p')
		else
		...
		if [[ -z $CMD3 ]]; then
			HELP=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"' (.*) \w{3,} \w{3,}( .*|$)/\1/p')
		elif [[ -n $CMD3 && -n $CMD4 ]]; then
			HELP=$(sed <<<"$HELP" -En "s/$CMD $CMD2.* $CMD3 $CMD4(.*)/\1/p")
		else
			HELP=""
		fi
...
_vboxmanage_words() {
	...
	if [[ $CMD2 == list ]]; then
		WORDS=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"'|--\w+//g; s/\[|]|\|/\n/g; p')
		...
		if [[ -z $CMD3 ]]; then
			WORDS=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"' [^ ]+ ('"$RE"*')( .*|$)/\1/p')
		...
		WORDS=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"' .* ('"$RE"'{2,})( .*|$)/\1/p')
		...
		if [[ -z $CMD3 ]]; then
			WORDS=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"'.* ('"$RE"'{2,}) '"$RE"'{2,}( .*|$)/\1/p')
		elif [[ -z $CMD4 ]]; then
			WORDS=$(sed <<<"$HELP" -En 's/'"$CMD $CMD2"'.* '"$CMD3"' ('"$RE"'{2,})( .*|$)/\1/p')
		fi
